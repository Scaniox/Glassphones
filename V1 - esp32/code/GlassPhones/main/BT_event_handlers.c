...
const static char* GAP_TAG = "BT_GAP";
...
void gap_evt_handler(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *parameters){
    switch(event){
        ...
        case ESP_BT_GAP_DISC_STATE_CHANGED_EVT:{
            ESP_LOGI(GAP_TAG, "discovery state changed to %i\n", parameters->disc_st_chg.state);
            break;
        }
...
void a2d_dat_evt_handler(const uint8_t *buf, uint32_t len){
    ...
    size_t written;
    i2s_write(max9850e_i2s_port, buf, len, &written, portMAX_DELAY);
}
