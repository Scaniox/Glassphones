...
void loop()
{
  if (Serial.available() > 0)
  {
    ...
    break;
    case 'l':
      if (BT.readLocalDeviceName())
        Serial.print("local device name: ");
      Serial.println(BT.localDeviceName);
      break;
    case 'L':
    {
      delay(1000);
...
void moduleInfo()
{
  ...
  */
  Serial.println();
  Serial.println(F("       UART version: "));
  Serial.println(BT.btmUartVersion, HEX);
  Serial.print(F("        flash version: "));
  Serial.println((uint8_t)(BT.btmUartVersion >> 13), HEX);
  Serial.print(F("          rom version: "));
  Serial.println((uint8_t)((BT.btmUartVersion >> 8) & 0x1F), HEX);
  Serial.print(F("    flash sub version: "));
  Serial.println(((uint8_t)(BT.btmUartVersion >> 4) & 0x0F), HEX);
  Serial.print(F("flash control version: "));
  Serial.println((uint8_t)(BT.btmUartVersion & 0x0F), HEX);
  Serial.println();
  Serial.println(F("      BT FW version: "));
  Serial.println(BT.btmFwVersion >> 13, HEX);
  Serial.print(F("        flash version: "));
  Serial.println((uint8_t)(BT.btmFwVersion >> 13), HEX);
  Serial.print(F("          rom version: "));
  Serial.println((uint8_t)((BT.btmFwVersion >> 8) & 0x1F), HEX);
  Serial.print(F("    flash sub version: "));
  Serial.println(((uint8_t)(BT.btmFwVersion >> 4) & 0x0F), HEX);
  Serial.print(F("flash control version: "));
  Serial.println((uint8_t)(BT.btmFwVersion & 0x0F), HEX);
}
...
void deviceInfo(uint8_t deviceId)
{
  ...
  Serial.print(F("info:\n"));
  Serial.print(BT.connectionStatus(deviceId));
  Serial.println();
  Serial.print(F("Name: "));
  Serial.println(BT.deviceName[deviceId]);
  Serial.print(F("Music Status: "));
  Serial.println(BT.musicStatus(deviceId));
  Serial.print(BT.streamStatus(deviceId));
  Serial.println();
  Serial.print(F("Battery level dev0: "));
  Serial.print(BT.currentBatteryLevel[deviceId]);
  Serial.print("/");
  Serial.println(BT.maxBatteryLevel[deviceId]);
  Serial.print(F("current signal strength : "));
  Serial.print(BT.currentSignalLevel[deviceId]);
  Serial.print("/");
  Serial.println(BT.maxSignalLevel[deviceId]);
  //    Serial.print(F(" info:");
