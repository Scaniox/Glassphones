...
BTAPP_STATUS BTAPP_Status;        //BTM system status
...
#endif
BT_AT_MODE BTAPP_AT_Mode = AT_TX;
AT_SOURCE_INPUT BTAPP_AT_Source = TX_SOURCE_AUX;
AT_SAMPLING_RATE BTAPP_AT_SamplingRate = TX_SAMPLING_RATE_48KHZ;

uint8_t BT_pwrOnInquiry;
uint8_t BT_TxConnectedDevices;
Searched_Result found_dev;
...
void BTAPP_Task(void) {
    switch (BTAPP_TaskState) {
        ...
        case BT_STATE_READ_PAIR_RECORD_WAIT:
            ...
            break;
		case BT_STATE_TX_PAIRING_KEY_WAIT:
            if (!BTAPP_timer1ms) {              //time out
				BTAPP_TaskState = BT_STATE_LINKBACK_START;
            }
		break;
        case BT_STATE_LINKBACK_START:
			// Power on link back only happen on AT RX
			// Require to press pairing button to link back on AT TX
            if((BT_IsCommandSendTaskIdle()))
            {
            	if(BTAPP_AT_Mode == AT_RX)
            	{
				...
				if(BTAPP_AT_Mode == AT_TX)
				{
					// Inquiry mode
					if(BT_pwrOnInquiry)
					{
						BT_pwrOnInquiry = 0;
						if(BTAPP_Status.status == BT_SYSTEM_STANDBY)
						{
							BTAPP_EnterInquiryMode();
						}
					}
					// Link back
					else if(BTAPP_Status.pairedRecordNumber != 0)
					{
						BTAPP_Status.status = BT_SYSTEM_LINK_BACK;
						BT_LinkBackToLastDevice();					
					}
					else
					{
						// Nothing to do
						// Just go to BT_STATE_BT_RUNNING 
					}
				}
...
uint8_t BTAPP_GetStatus(void)
{
	...
	if(BTAPP_TaskState == BT_STATE_POWER_OFF)
        ...
        return BT_STATUS_ON;
			...
			BTAPP_TaskState == BT_STATE_BT_AT_CONNECT
		    )
...
void BTAPP_EventHandler(BT_APP_EVENTS event, uint8_t* paras, uint16_t size )
{
    ...
    switch(event)
    {
        ...
        case BT_EVENT_CMD_SENT_ACK_ERROR:
			...
			if(paras[0] == PROFILE_LINK_BACK)
			{
				if(BTAPP_Status.status == BT_SYSTEM_LINK_BACK)
        ...
        case BT_EVENT_A2DP_LINK_CHANGED:
			if(BTAPP_TaskState == BT_STATE_BT_AT_CONNECT)
			{
				BTAPP_ConnectNextSink();
			}
            BTAPP_Status.status = BT_SYSTEM_CONNECTED;
	   ...
	   case BT_EVENT_NAME_RECEIVED:
        ...
        case BT_EVENT_PAIR_RECORD_RECEIVED:
            ...
            {
        		BTAPP_timer1ms = KEY_SECONDARY_HOLD_DELAY;
                BTAPP_TaskState = BT_STATE_TX_PAIRING_KEY_WAIT;
            }
            break;
        ...
        case BT_EVENT_REPORT_TYPE_CODEC:
            ...
            break;
		case BT_EVENT_AT_SOURCE_CHANGE:
			BTAPP_AT_Source	= paras[1];
		break;
		case BT_EVENT_AT_MODE_CHANGE:
			BTAPP_AT_Mode = paras[1];
		break;
		case BT_EVENT_AT_SAMPLING_RATE_CHANGE:
			BTAPP_AT_SamplingRate = paras[1];
		break;
		case BT_EVENT_INQUIRY_RESPONSE:
			BTAPP_ExtractInqurySinkInfo(paras+1);
		break;			
		case BT_EVENT_INQUIRY_COMPLETE:		
			// Start to connect the inquired devices
			if(paras[1] == 0)
			{
				// Success
				BT_BlockTxA2DPStream(true);
				found_dev.tryConnectNum = found_dev.dev_num;
				BTAPP_ConnectNextSink();
			}
			else
			{
				// Failure , please refer API document for failure code
				BTAPP_TaskState = BT_STATE_BT_RUNNING;
			}
		break;
		case BT_EVENT_ACL_CONNECTED:
			if(BTAPP_AT_Mode == AT_TX)
			{
				++BT_TxConnectedDevices;
				if(BTAPP_Status.pairedRecordNumber==0)
					BTAPP_Status.pairedRecordNumber = BT_TxConnectedDevices;
			}
		break;
		...
		case BT_EVENT_INQUIRY_STARTED:
				BTAPP_Status.status = BT_SYSTEM_INQUIRY;
        default:
            break;
    }
}
...
/*-----------------------------------------------------------------------------*/
/*-----------------------------------------------------------------------------*/
void BTAPP_ResetSinksInfo()
{
    memset(&found_dev,0, sizeof(found_dev));
}
...
void BTAPP_ExtractInqurySinkInfo(uint8_t* rcvd_buf)
{
    ...
    if(found_dev.dev_num >= INQUIRY_DEVICES_NUM)
		return;
    index = found_dev.dev_num;
    for(i=0;i<6;i++)
    {
        found_dev.dev_id[index].bt_address[i] = *rcvd_buf++;
    }
    found_dev.dev_id[index].class_of_dev[0] = *rcvd_buf++;
    found_dev.dev_id[index].class_of_dev[1] = *rcvd_buf++;
    found_dev.dev_id[index].class_of_dev[2] = *rcvd_buf++;
	found_dev.dev_id[index].rssi_val = *rcvd_buf++;
	// Dont handle Extended inquiry response here
    found_dev.dev_num++;
    found_dev.dev_id[index].toConnect = false;

}
// Find a best RSSI sink device to connect
bool BTAPP_ConnectNextSink(void)
{
    uint8_t i=0, bestRssi = 0, bestRssiDevId=TX_MAX_SINK_DEVICES_NUM;
	// Check if there are already two devices connected 
	// Check if it already tried all devices
	if( (BT_TxConnectedDevices >= TX_MAX_SINK_DEVICES_NUM) ||
		(found_dev.tryConnectNum == 0))
    {
    	// Start music playback when there is at least one connected device
    	if(BT_TxConnectedDevices > 0)
			BT_BlockTxA2DPStream(false);
   		BTAPP_TaskState = BT_STATE_BT_RUNNING;
		return false;
    }
	// Search in the table to find a best RSSI
	// If "toConnect" is true, the devices have been tried to connect
	// Only look at the devices have not been tried

    for(i=0;i<found_dev.dev_num;i++)
    {
        if(found_dev.dev_id[i].toConnect==false)
        {
	        if(found_dev.dev_id[i].rssi_val >= bestRssi)
	        {
				bestRssi = found_dev.dev_id[i].rssi_val;
				bestRssiDevId = i;
	        }
        }
	}
	
	if(bestRssiDevId != TX_MAX_SINK_DEVICES_NUM)
	{
		// Try one more devices
		found_dev.tryConnectNum--;
		// This devices will be tried
		found_dev.dev_id[bestRssiDevId].toConnect=true;
		BTAPP_TaskState = BT_STATE_BT_AT_CONNECT;
		BT_Connect_Device(found_dev.dev_id[bestRssiDevId].bt_address);
		return true;
	}
	else
	{
		// A condition if all RSSI are zero, should not possible.
		BTAPP_TaskState = BT_STATE_BT_RUNNING;
		return false;
	}
}
...
void BTAPP_CheckEnterInquiryMode( void )
{
	if(BTAPP_TaskState <= BT_STATE_BT_RUNNING)
	{
		...
		{
			// Once BTAPP_TaskState enter "BT_STATE_BT_RUNNING",
			// if BTAPP_Status.status == BT_SYSTEM_STANDBY,
			// it will enter inquiry mode
			BT_pwrOnInquiry = 1;
		}
	}
	else
...
{
	Set_LED_Style(LED_1, LED_BLINK, 500, 500);		//1HZ before pairing starting
	BTAPP_ResetSinksInfo();
	BT_EnterInquiryMode();
}
void BTAPP_Toggle_A2DP(void)
{
    BT_MMI_ActionCommand (TOGGLE_AUDIO_PLAYBACK, 0);
}
BT_AT_MODE BTAPP_GetATMode(void)
{
	return BTAPP_AT_Mode;
}

AT_SOURCE_INPUT BTAPP_GetTxSource(void)
{
	return BTAPP_AT_Source;
}

AT_SOURCE_INPUT BTAPP_GetTxSamplingRate(void)
{
	return BTAPP_AT_SamplingRate;
}
void BTAPP_HandlePairingShortPress(void)
{
	...
	{
		switch(BTAPP_Status.status)
		{
			case BT_SYSTEM_STANDBY:		
				//	initiate link back by 0x17
				//	a:	0x17 0x00 to link back last connected devices
				//	b:	0x17 0x04 XX where XX is device index		
				if(BTAPP_Status.pairedRecordNumber !=0)
				{
					BTAPP_Status.status = BT_SYSTEM_LINK_BACK;
					BT_LinkBackToLastDevice();
			...
			case BT_SYSTEM_CONNECTED:	
				...
				BTAPP_Toggle_A2DP();
			break;
			case BT_SYSTEM_INQUIRY:					
				//	Cancel inquiry
				//	However, BTM status do not contains "Inquiry" status
				//	SW team will include it in next release
				Set_LED_Style(LED_1, LED_OFF, 0, 0);		//1HZ before pairing starting				
				BT_CancelInquiryMode();	
			break;
			case BT_SYSTEM_LINK_BACK:
				// Cancel link back
				BTAPP_TaskState = BT_STATE_BT_RUNNING;
			break;
		}
